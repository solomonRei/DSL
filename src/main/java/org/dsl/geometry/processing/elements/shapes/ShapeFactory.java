...
public class ShapeFactory {
    ...
    public static Triangle createTriangleWithAliases(Map<String, Float> sides) {
        ...
        Point p2 = new Point(Settings.INITIAL_COORDINATE_X + side1Entry.getValue(), Settings.INITIAL_COORDINATE_Y, side2Entry.getKey());

        double angle = Math.acos((side1Entry.getValue() * side1Entry.getValue() + side3Entry.getValue() * side3Entry.getValue() - side2Entry.getValue() * side2Entry.getValue()) / (2 * side1Entry.getValue() * side3Entry.getValue()));
        float px = (float) (side3Entry.getValue() * Math.cos(angle));
        float py = (float) (side3Entry.getValue() * Math.sin(angle));

        Point p3 = new Point(Settings.INITIAL_COORDINATE_X + px, Settings.INITIAL_COORDINATE_Y + py, side3Entry.getKey());
